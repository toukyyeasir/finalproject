...
if($_SERVER["REQUEST_METHOD"] == "POST") {
	...
	if( empty($password)) {
		...
		if($conn -> connect_error) {
			...
			$stmt = $conn -> prepare("INSERT INTO `companyreview` (`username`, `comment`) VALUES(?, ?)");
...
if($_SERVER["REQUEST_METHOD"] == "GET") {
	...
	$sql = "SELECT id, username, comment FROM companyreview WHERE id = " . $searchKey;

	if(empty($searchKey)) {
		$sql = "SELECT id, username, comment FROM companyreview";
	}
